...
import rubiks.lib.yoinked.optimal.defs as defs
from rubiks.lib.yoinked import optimal as enums, optimal as sy, optimal as cb
...
def create_phase1x24_prun_table():
    ...
    if not path.isfile(fname):
        ...
        for i in trange(defs.N_FLIPSLICESORTED_CLASS):
            # if (i + 1) % 24000 == 0:
...
def create_cornerprun_table():
    ...
    if not path.isfile(fname):
        ...
        while done != defs.N_CORNERS:
            for corners in trange(defs.N_CORNERS):
